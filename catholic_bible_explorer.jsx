...
export default function CatholicBibleApp() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (b?.value) setBookmarks(JSON.parse(b.value));
      } catch (e) {}
      try {
      ...
      try {
        ...
        if (s?.value) {
          const { theme: t, fontSize: f } = JSON.parse(s.value);
          if (t) setTheme(t);
          if (f) setFontSize(f);
        }
      } catch (e) {}
  ...
  const persistBookmarks = useCallback(async (next) => {
    try { await window.storage?.set('bookmarks', JSON.stringify(next)); } catch (e) {}
  }, []);
  const persistHistory = useCallback(async (next) => {
    try { await window.storage?.set('history', JSON.stringify(next)); } catch (e) {}
  }, []);
  useEffect(() => {
    (async () => {
      try { await window.storage?.set('settings', JSON.stringify({ theme, fontSize })); } catch (e) {}
    })();
  }, [theme, fontSize]);
